...
def explain(req: ExplainRequest):
    ...
    exp = explainer.explain_instance(clean_text, predict_proba_batch, num_features=req.num_features, num_samples=req.num_samples, labels=[0, 1])
    reasons = [{"word": w.strip('.,!?;:()"'), "weight": round(wt, 4)} for w, wt in exp.as_list(label=predicted_class)]
    return {
        "model_prediction": {"not_credible": round(float(probs[0]), 4), "credible": round(float(probs[1]), 4)},
