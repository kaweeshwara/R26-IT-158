...
def _build_reasons(
    *,
    ...
    if label == "Recent":
        reasons.append("Content is recent but not brand-new (within the last month)")
    elif label == "Aging":
        reasons.append("Content is aging (over a month old) — verify it is still current")
    elif label == "Old":
        reasons.append("Content is old (over a year old)")
    elif label == "Very Old":
        reasons.append("Content is outdated (over 3 years old)")
    elif label == "Unknown":
